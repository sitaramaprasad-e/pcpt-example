...
if [[ "$USE_PROXY" == true ]]; then
  ...
  if docker inspect $CONTAINER_NAME >/dev/null 2>&1; then
    docker inspect --format '🔍 TRACE: App container Health={{ (index .State "Health").Status }}' $CONTAINER_NAME 2>/dev/null || true
  fi
else
...
if docker inspect $PROXY_NAME >/dev/null 2>&1; then
  docker inspect $PROXY_NAME --format '🔍 TRACE: Caddy container IP={{range.NetworkSettings.Networks}}{{.IPAddress}}{{end}}' || true
  docker port $PROXY_NAME | sed 's/^/🔍 TRACE: /' || true
fi

  if [ $? -ne 0 ]; then
    echo "❌ Failed to start Caddy proxy!"
    echo "🔍 TRACE: Caddy logs (last 100 lines):"
    docker logs --tail 100 $PROXY_NAME 2>/dev/null || true
    exit 1
  fi
...
if docker inspect $PROXY_NAME >/dev/null 2>&1; then
  ...
  if [[ "$INTERNAL_DOMAIN_TLS" == true ]]; then
    docker exec $PROXY_NAME sh -c "apk add --no-cache openssl >/dev/null 2>&1 || true; openssl s_client -connect localhost:443 -servername '$DOMAIN' </dev/null 2>/dev/null | openssl x509 -noout -subject -issuer -dates" || true
  elif [[ "$LOCAL_TLS" == true ]]; then
    docker exec $PROXY_NAME sh -c "apk add --no-cache openssl >/dev/null 2>&1 || true; openssl s_client -connect localhost:443 -servername 'localhost' </dev/null 2>/dev/null | openssl x509 -noout -subject -issuer -dates" || true
  else
    docker exec $PROXY_NAME sh -c "apk add --no-cache openssl >/dev/null 2>&1 || true; openssl s_client -connect localhost:443 -servername '$DOMAIN' </dev/null 2>/dev/null | openssl x509 -noout -subject -issuer -dates" || true
  fi
fi
...
echo "💡 TRACE tip: to follow logs, run: docker logs -f $PROXY_NAME (proxy) or docker logs -f $CONTAINER_NAME (app)"